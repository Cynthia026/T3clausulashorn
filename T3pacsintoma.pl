...
puede_recetarse(Paciente, Medicamento) :- esta_enfermo(Paciente, Enfermedad), alivia(Medicamento, Enfermedad).
...
tiene_multiples_sintomas(Paciente) :-tiene_sintoma(Paciente, S1), tiene_sintoma(Paciente, S2),S1 \= S2.
...
medicamento_util(Medicamento) :-elimina(Medicamento, Sintoma),tiene_sintoma(_, Sintoma).


% 8. Un paciente tiene síntomas sin diagnóstico conocido
sintoma_sin_diagnostico(Paciente, Sintoma) :-tiene_sintoma(Paciente, Sintoma),\+ sintoma_de(Sintoma, _).
